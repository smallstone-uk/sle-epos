<cfcomponent displayname="EPOS" hint="version 14. EPOS Till Functions">
		...
		<cftry>
			...
			<cfswitch expression="#args.form.itemClass#">
				...
				<cfcase value="PRIZE">
					...
					<cfset args.form.btnSend = "PRIZE">
					<cfset AddPayment(args)>
				</cfcase>
				<cfcase value="VCHN">
					<!--- pass onto payment routine --->
					<cfset args.form.btnSend = "VCHN">
					<cfset AddPayment(args)>
<!---					<cfif ArrayLen(session.basket.supplier) gt 0> <!--- already have supplier transaction in basket --->
					...
					<cfelse>
						...
						<cfif args.data.cash neq 0>
							...
							<cfset args.data.type = args.form.itemClass>
							<cfset CalcValues(args.data)>
							<cfif args.form.addToBasket><cfset ArrayAppend(session.basket.vchn,args.data)></cfif>
							<!---<cfset CalcTotals()>--->
						</cfif>
					</cfif>
--->				</cfcase>
				<cfcase value="CPN">
					<cfset args.form.btnSend = "CPN">
					<cfset AddPayment(args)>
<!---					<cfif ArrayLen(session.basket.supplier) gt 0> <!--- already have supplier transaction in basket --->
					...
					<cfelse>
						...
						<cfif args.data.cash neq 0>
							...
							<cfset args.data.type = args.form.itemClass>
							<cfset CalcValues(args.data)>
							<cfif args.form.addToBasket><cfset ArrayAppend(session.basket.cpn,args.data)></cfif>
							<!---<cfset CalcTotals()>--->
						</cfif>
					</cfif>
--->				</cfcase>			
		...
		<cftry>
			...
			<cfswitch expression="#args.form.btnSend#">
				...
				<cfcase value="VCHN">
					...
					<!---<cfset args.data.title = "Voucher Payment">--->
					<cfset args.data.account = 2>
					<cfset args.data.prodID = 2>
					<cfset ArrayAppend(session.basket.payments,args.data)>
				</cfcase>
				<cfcase value="CPN">
					...
					<!---<cfset args.data.title = "Coupon Payment">--->
					<cfset args.data.account = 2>
					<cfset args.data.prodID = 2>
					<cfset ArrayAppend(session.basket.payments,args.data)>
				</cfcase>
				<cfcase value="PRIZE">
					...
					<!---<cfset args.data.title = "Lottery Prize">--->
					<cfset args.data.account = 2>
					<cfset args.data.prodID = 2>
					<cfset ArrayAppend(session.basket.payments,args.data)>
				</cfcase>
				...
				<table class="eposBasketTable" border="0" width="100%">
					...
					<cfloop array="#session.till.catKeys#" index="loc.key">
						...
						<cfloop array="#loc.section#" index="loc.item">
							...
							<cfset session.basket.header.bCash += loc.data.cash>
							<cfset session.basket.header.bCredit += loc.data.credit>
							<cfset session.basket.header.balance -= (loc.data.cash + loc.data.credit)>
					...
					<tr class="ebt_headers">
						...
						<th align="center"></th>
						<th></th>
						<th align="right">#DecimalFormat(session.basket.total.discount)#</th>
						<th></th>
					</tr>
				</table>
			</cfoutput>
