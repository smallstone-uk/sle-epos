	...
	<cffunction name="LoadProducts" access="public" returntype="query">
		...
		<cftry> 
			<cfquery name="loc.QProducts" datasource="#args.datasource#" result="loc.QProductsResult">
				...
				(SELECT prodID,prodRef,prodTitle,prodOurPrice,prodVATRate,prodCashOnly,prodStaffDiscount
				FROM tblProducts
				WHERE prodID IN (25822,25882,25892) )	
			</cfquery>
	...
	<cffunction name="LoadPapers" access="public" returntype="query">
		...
		<cftry> 
			<cfquery name="loc.QPapers" datasource="#args.datasource#">
				SELECT pubID,pubTitle,pubPrice,pubTradePrice
				FROM `tblPublication` 
				WHERE `pubGroup` = 'News' 
				AND `pubEPOS` = 1
			</cfquery>
...
<cfobject component="code/epos12" name="ecfc">
...
<cfoutput>
	<div id="tillpanel">
		...
		<form name="basket" method="post" enctype="multipart/form-data">
			<div style="width:100%">
				Products: <select name="type" id="productType">
					<option value="">Select...</option>
					<cfloop query="session.products">
						<option value="prod-#prodID#" data-price="#prodOurPrice#" data-cashonly="#prodCashOnly#" data-prodid="#prodID#" 
				...
				<br>
				Newspapers: <select name="pubID" id="mediaType">
					<option value="">Select...</option>
					<cfloop query="session.papers">
						<option value="#pubID#" data-price="#pubPrice#" data-cashonly="0" data-staffDiscount="1" data-title="#pubTitle#" data-vatrate="0">
							0 - #pubID# - #Left(pubTitle,20)# - #pubPrice# - 0%</option>
					</cfloop>
				</select>
				<input type="checkbox" name="discountable" id="discountable" />Discountable
				<br>
				<input name="addToBasket" type="hidden" value="true" />
				<input name="prodID" id="prodID" type="text" value="" />
				<input name="prodTitle" id="prodTitle" type="text" value="" />
				<input name="pubID" id="pubID" type="hidden" value="" />
				<input name="pubTitle" id="pubTitle" type="hidden" value="" />
