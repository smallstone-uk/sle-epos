...
<cfoutput>
	...
	<cfloop array="#new App.ProductGroup().all()#" index="pGroup">
		<cfif FileExists("#getBaseDir('Events/#pGroup.pgClassname#.js')#") AND NOT arrayContains(usedEvents, pGroup.pgClassname)>
			...
			<script>
				var classname = '#lCase(pGroup.pgClassname)#';
	...
	<cfset employees = epos.LoadEmployees()>	<!---StructKeyExists(session,"user") AND StructKeyExists(session.user,"prefs") AND  --->
	<cfif StructKeyExists(session.user.prefs,"empBackground")>
		<cfset backGround = session.user.prefs.empBackground>
		<cfset loggedIn = session.user.loggedin>
	<cfelse>
		<cfset backGround = "default.jpg">
			...
			$(document).ready(function(e) {
				...
				<!---<cfif StructKeyExists(session.user.prefs, "empBackground")>--->
					...
					console.log("logged in " + loggedin);
						...
						// start idle counter					
						$(document).on('keypress touchstart touchend touchmove click', function () {
							console.log('activity');
							clearInterval(warningCounter);
							restartCounter();
						});
						
						// warn user if basket contains items and been idle for specified period
						function restartCounter() {
							warningCounter = setTimeout(() => {
								console.log("restart counter");
								$.ajax({
									type: "POST",
									url: "ajax/idleCheck.cfm",
									data: {},
									success: function(data) {
										$('##idleCounter').html(data);
										if (data != 0) {
											sound('warning');
											clearInterval(warningCounter);
											restartCounter();
							...
							}, #application.settings.idleTimeout#);
						}
							...
							screensaver = setTimeout(function() {
								...
								$('body').prepend(
									'<div class="ss_wrapper anim_slidein in">'+
										...
										'<div class="screensaver" style="background-image:url(../images/wallpapers/#backGround#);"></div>'+
									'</div>'
								);
